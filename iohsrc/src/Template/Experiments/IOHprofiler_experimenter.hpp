...
public:
  ...
  IOHprofiler_experimenter(std::string configFileName, std::string algoname, _algorithm *algorithm) {
    ...
    std::string x = (algoname == ""? conf.get_algorithm_name():algoname );
    std::shared_ptr<IOHprofiler_csv_logger> logger(new IOHprofiler_csv_logger(conf.get_output_directory(),x,x,x));
    if (logger == nullptr) {
